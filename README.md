...
- [AWS DevOps Agent Integration](#aws-devops-agent-integration)
  ...
  - [Start an Investigation](#starting-an-investigation)
- [Fault Injection Scenarios](#fault-injection-scenarios)
  - [Catalog Service Latency](#1-catalog-service-latency-injection)
  - [Network Partition](#2-network-partition)
  - [RDS Security Group Block](#3-rds-security-group-misconfiguration)
  - [Cart Memory Leak](#4-cart-memory-leak)
  - [DynamoDB Stress Test](#5-dynamodb-stress-test)
- [Cleanup](#cleanup---destroying-the-lab-environment)
...
1. **AWS GuardDuty** automatically creates VPC endpoints and security groups for runtime monitoring - these block subnet/VPC deletion
...
The script will:
1. Get VPC ID for the cluster
2. Delete VPC endpoints and GuardDuty security groups (prevents subnet deletion failures)
3. Remove Kubernetes resources from Terraform state (prevents provider errors)
4. Run `terraform destroy` to remove all Terraform-managed resources
5. Final VPC cleanup if it still exists
6. Clean up orphaned CloudWatch log groups
...
| Dependency | License |
|------------|---------|
| MariaDB Community Edition | [LICENSE](https://mariadb.com/kb/en/mariadb-license/) |
| MySQL Community Edition | [LICENSE](https://www.mysql.com/about/legal/licensing/oem/) |
